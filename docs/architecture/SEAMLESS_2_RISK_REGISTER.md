...
| R12 High: validation coverage | CI runs PowerShell tests only. C unit tests do not form a reconnect harness. The owner manually observed DEV video/control, USB operation with Wi-Fi disabled and visual USB-to-Wi-Fi failover. PID/HWND continuity, audio recovery, repeated cycles and full synchronized E2E evidence remain unverified. | Preserve the 24-suite/14-test/package evidence and the separately attributed manual observations; do not extrapolate a visual window check to process identity. | 2/5/7/12: build CI, UI/IPC/native harnesses and sanitized hardware/fault/soak reports. See hardware procedure. |
