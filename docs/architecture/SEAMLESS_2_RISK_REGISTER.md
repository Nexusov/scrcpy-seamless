...
| R12 High: validation coverage | CI runs PowerShell tests only. C unit tests do not form a reconnect harness. No phone mirroring, server tests, full Start-to-mirroring E2E or repeated hardware recovery in this run. | Separate 24-suite, 14-test, package and UI/discovery results; list pending hardware cases instead of extrapolating. | 2/5/7/12: build CI, UI/IPC/native harnesses and sanitized hardware/fault/soak reports. See hardware procedure. |
