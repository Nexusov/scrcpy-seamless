...
| Android build dependencies | [`server-toolchain.json`](../../scripts/server-toolchain.json), checked-in Gradle wrapper and Gradle files | Fixed JDK and SDK-tool archive checksums plus Gradle, plugin, API and test-tool pins. The server build checks its pins before compiling. |
