# Development policies

These documents are canonical operational rules for contributors and coding
agents.

- [Git workflow](git-workflow.md)
- [Debugging and root-cause analysis](debugging.md)
- [Release process](release-process.md)
- [Recommended GitHub repository settings](repository-settings.md)

The root `AGENTS.md` is intentionally a short map and points here instead of
duplicating these policies.

When a policy changes, update the canonical document first, then update
`AGENTS.md` only if its concise summary is no longer correct.
